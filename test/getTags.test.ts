import { getTags, getTokens } from '../src';
...
describe('getTags', () => {
    ...
    it('should not take spaces for tag name', () => {
        const tokens = getTokens('create a separate file file name:file-02.txt');
        const result = getTags(tokens);
        expect(result).toEqual([{name: 'name', value: 'file-02.txt'}]);
    });

    it('should not take spaces for tag value', () => {
        const tokens = getTokens('create a separate file file-name:file name.txt');
        const result = getTags(tokens);
        expect(result).toEqual([{name: 'file-name', value: 'file'}]);
    });

    it('should return multiple tags', () => {
        const tokens = getTokens('watch a new episode of law and order episode:05 season:02');
        const result = getTags(tokens);
        expect(result).toEqual([{name: 'episode', value: '05'}, {name: 'season', value: '02'}]);
    });

    it('should return multiple tags scattered', () => {
        const tokens = getTokens('id:t1 make a search for topic:network-security for school, course:computer-networks');
        const result = getTags(tokens);
    ...
    it('should handle multiple tags with same name and value', () => {
        const tokens = getTokens('x @test setup test for tag:test-101 repeated tag:test-101');
        const result = getTags(tokens);
        expect(result).toEqual([
